...
export default function Assets() {
  const assets = useAppSelector((s) => s.workspace.assets);
        ...
        {assets.map((a) => {
          ...
          return (
            <Card key={a.id} className="p-5 shadow-soft">
              ...
              {a.serial && (
                <p className="mt-2 font-mono text-xs text-muted-foreground">SN: {a.serial}</p>
              )}
              {a.assignedOn && (
                <p className="text-xs text-muted-foreground">Assigned: {a.assignedOn}</p>
              )}
            </Card>
          );
        })}
