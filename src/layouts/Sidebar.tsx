  ...
  return (
    <aside className={cn(
      ...
      <div className="flex h-16 items-center gap-3 border-b px-4">
        <div className={cn("flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br text-white shadow-glow", meta.accent)}>
        ...
        {!collapsed && (
          <div className="min-w-0">
            ...
            <div className="truncate text-[10px] uppercase tracking-wider text-muted-foreground">{meta.title}</div>
          </div>
        )}
      </div>
            ...
            <li key={item.to}>
              ...
              >
                {({ isActive }) => (
                  <>
                    {isActive && (
                      <motion.div layoutId={`active-${role}`} className="absolute left-0 h-6 w-1 rounded-r-full bg-primary" />
                    )}
