  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {holidays.map((h) => (
          <Card key={h.id} className="overflow-hidden p-0 shadow-soft transition-transform hover:-translate-y-0.5">
            <div className="flex items-center gap-4 border-b bg-gradient-to-br from-primary/10 to-transparent p-4">
              <div className="flex h-14 w-14 flex-col items-center justify-center rounded-xl bg-card shadow-soft">
                <span className="text-[10px] uppercase text-muted-foreground">{new Date(h.date).toLocaleString("en", { month: "short" })}</span>
            ...
            <div className="flex items-center justify-between p-4">
              <span className="flex items-center text-xs text-muted-foreground"><CalendarDays className="mr-1 h-3.5 w-3.5" />Holiday</span>
