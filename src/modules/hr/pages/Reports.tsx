        ...
        {reports.map((r) => (
          <Card key={r.id} className="flex items-center justify-between p-5 shadow-soft">
            <div className="flex items-center gap-3">
              ...
              <div><h3 className="font-semibold">{r.name}</h3><p className="text-xs text-muted-foreground">{r.desc}</p></div>
            </div>
            <div className="flex gap-2"><ExportButtons rows={employees} filename={r.name.replace(/\s+/g,"_").toLowerCase()} /></div>
          </Card>
        ))}
      </div>
    </div>
