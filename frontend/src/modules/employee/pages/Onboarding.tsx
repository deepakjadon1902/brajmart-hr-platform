      ...
      <Card className="p-6 shadow-soft">
        ...
        <ul className="mt-6 space-y-3">
          {steps.map((s) => (
            ...
            >
              <div
                className={`flex h-7 w-7 items-center justify-center rounded-full ${s.done ? "bg-success text-success-foreground" : "bg-muted text-muted-foreground"}`}
              >
                {s.done ? <Check className="h-4 w-4" /> : <span className="text-xs">•</span>}
              </div>
              <span className={s.done ? "line-through text-muted-foreground" : "font-medium"}>
                {s.label}
              </span>
            </li>
          ))}
