...
export function AuthShell({ role }: { role: Role }) {
  ...
  const onSubmit = async (v: FormValues) => {
    try {
      ...
      navigate(location.state?.from?.pathname || meta.home, { replace: true });
    } catch (e: any) {
  ...
  return (
    <div className="relative grid min-h-dvh w-full md:grid-cols-2">
      <div className={`relative hidden overflow-hidden md:flex md:flex-col md:justify-between bg-gradient-to-br p-10 text-white ${meta.accent}`}>
        <div className="absolute inset-0 opacity-20" style={{
          backgroundImage: "radial-gradient(white 1px, transparent 1px)", backgroundSize: "22px 22px",
        }} />
        <div className="relative flex items-center gap-2">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-white/15 backdrop-blur">
            <Sparkles className="h-5 w-5" />
          </div>
          <span className="text-lg font-bold">Pulse HRMS</span>
        </div>
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="relative">
          ...
          <div className="mt-8 grid grid-cols-2 gap-3">
            {["Secure SSO", "Mobile-first", "Real-time", "Dark mode"].map((f) => (
              <div key={f} className="flex items-center gap-2 rounded-xl bg-white/10 p-3 text-sm backdrop-blur">
                <ShieldCheck className="h-4 w-4" /> {f}
              </div>
            ))}
          </div>
        </motion.div>
        <p className="relative text-xs text-white/70">© {new Date().getFullYear()} Pulse HRMS</p>
      </div>
      ...
      <div className="relative flex items-center justify-center p-6 md:p-12">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
          ...
          <div className="rounded-3xl border bg-card p-7 shadow-elevated">
            ...
            <p className="mt-1 text-sm text-muted-foreground">Use your demo credentials — they're pre-filled for you.</p>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-4">
              ...
              <div>
                ...
                <div className="relative mt-1">
                  <Input id="password" type={show ? "text" : "password"} autoComplete="current-password" {...register("password")} className="pr-10" />
              ...
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" {...register("remember")} className="h-4 w-4 rounded border accent-[color:var(--primary)]" />
              ...
              <Button type="button" variant="outline" className="w-full" onClick={() => { setValue("email", `${role.replace("-","")}@demo.com`); setValue("password","demo1234"); }}>
